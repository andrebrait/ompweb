...
test("behavioral: pause and resume accounting retains startedAt, accumulates paused time, and preserves analyser", () => {
  const fakeAnalyser = { getByteFrequencyData: () => {} };
  const capture = {
  ...
  // Simulate pause at t=4000ms
  capture.pausedAt = 4000;
  assert.equal(capture.pausedAt, 4000);
  assert.equal(capture.analyser, fakeAnalyser);

  // Simulate teardownPreviewAudio does NOT touch captureRef
  let previewRevoked = false;
  const teardownPreviewAudio = () => {
    previewRevoked = true;
  };
  teardownPreviewAudio();
  assert.equal(previewRevoked, true);
  assert.equal(capture.startedAt, 1000); // startedAt NOT wiped
  assert.equal(capture.analyser, fakeAnalyser); // analyser NOT nulled

  // Simulate resume at t=9000ms (5000ms pause)
  const resumeTime = 9000;
  const { pausedAt, pausedAccum } = capture;
  assert.notEqual(pausedAt, null);
  if (pausedAt !== null) {
    capture.pausedAccum = pausedAccum + (resumeTime - pausedAt);
  }
  capture.pausedAt = null;

  assert.equal(capture.pausedAccum, 5000);
  assert.equal(capture.pausedAt, null);
  assert.equal(capture.startedAt, 1000);
  assert.equal(capture.analyser, fakeAnalyser);

  // At t=10000ms, active elapsed time must be 10000 - 1000 - 5000 = 4000ms
  const now = 10000;
  const elapsed = now - capture.startedAt - capture.pausedAccum - (capture.pausedAt !== null ? now - capture.pausedAt : 0);
  assert.equal(elapsed, 4000);
});

test("behavioral: teardownPreviewAudio revokes object URL and pauses audio element", () => {
  let revokedUrl = null;
  let paused = false;
  ...
  // Simulated teardownPreviewAudio
  const teardown = () => {
    fakeAudio.pause();
    fakeAudio.src = "";
    fakeUrlApi.revokeObjectURL(fakeUrl);
  };

  teardown();
  assert.equal(paused, true);
  assert.equal(fakeAudio.src, "");
  assert.equal(revokedUrl, fakeUrl);
});

test("behavioral: RecordingDeck pause button renders Play when paused and Pause when active", async () => {
  const deckSource = await readFile(new URL("../components/RecordingDeck.tsx", import.meta.url), "utf8");

  // Assert that isPaused renders Play icon, not Pause in both branches
  assert.match(deckSource, /isPaused \?\s*\(\s*<Play/);
});
