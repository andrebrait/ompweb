...
test("maps a skills_list RPC payload onto SkillInfo", () => {
  const mapped = skillsFromCliPayload({
    ...
    warnings: [{ skillPath: "", message: "boom" }],
  });
  ...
  assert.deepEqual(mapped.diagnostics, [{ skillPath: "", message: "boom" }]);
});

test("rejects a malformed skills_list payload", () => {
  assert.equal(skillsFromCliPayload(null), undefined);
  assert.equal(skillsFromCliPayload("nope"), undefined);
  assert.deepEqual(skillsFromCliPayload({ skills: "many", warnings: [] }), { skills: [], diagnostics: [] });
});

test("discoverSkills caches results in memory and invalidates on request", async () => {
  const dir = mkdtempSync(join(tmpdir(), "skills-cache-test-"));
  try {
    invalidateSkillsCache(dir);
    const first = await discoverSkills(dir);
    const second = await discoverSkills(dir);
    assert.equal(first, second);
    invalidateSkillsCache(dir);
    const third = await discoverSkills(dir);
